...
export default () => new Promise(async (resolve, reject) => {
  ...
  if (!fs.existsSync(`${destFliteFolder}/flite`)) {
    try {
      ...
      log.info('Unpacking...')
      await command(`cd ${tmpDir} && tar xfvj flite-2.1-release.tar.bz2 && cp ../assets/tridev.lv flite-2.1-release/config`, { shell: true })
      log.success('Unpack done')
      log.info('Configuring...')
      await command(`cd ${tmpDir}/flite-2.1-release && ./configure --with-langvox=tridev`, { shell: true })
